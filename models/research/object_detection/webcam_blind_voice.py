...
# Initialize the text-to-speech engine at the beginning
def speak_message(message):
    try:
        ...
        # Ensure some delay before stopping (not usually required)
        #time.sleep(0.5)  # Adjust delay based on length of message
...
last_distance_state = None  # Tracks the last state: 'safe' or 'warning'
  ...
  with tf.compat.v1.Session(graph=detection_graph) as sess:
   ...
   while (ret):
      ...
      for i,b in enumerate(boxes[0]):
        ...
        if classes[0][i] == 1:  # Check for class type (e.g., person)
            if scores[0][i] >= 0.5:  # Confidence threshold
                ...
                # Trigger voice command only if the state has changed
                if current_state != last_distance_state:
                    if current_state == "safe":
                        speak_message(f"{apx_distance} units")
                        speak_message("Person is AT A SAFER DISTANCE")
                    elif current_state == "warning":
                        if 0.3 < mid_x < 0.7:  # Ensure person is near center
                            cv2.putText(image_np, 'WARNING!!!', (50, 50),
                                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3)
                            print("Warning - Person very close to the frame")
                            speak_message("Warning - Person very close to the frame")

                # Update the last state
                last_distance_state = current_state
